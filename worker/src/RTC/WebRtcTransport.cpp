...
namespace RTC {
    ...
    WebRtcTransport::WebRtcTransport(const std::string &transportId, RTC::Transport::Listener *listener, json &data)
        ...
        if (jsonEnableOtherUdpIt != data.end()) {
            if (!jsonEnableOtherUdpIt->is_boolean()) {
                MS_THROW_TYPE_ERROR("wrong enableOtherUdp (not a boolean)");
            }
        ...
        if (!enableOtherUdp) {
            ...
            try {
                ...
                std::string udpIp;
                auto jsonUdpIpIt = data.find("otherUdpIp");
                if (jsonUdpIpIt == data.end())
                    MS_THROW_TYPE_ERROR("missing otherUdpIp");
                else if (!jsonUdpIpIt->is_string())
                    MS_THROW_TYPE_ERROR("wrong otherUdpIp (not an string");
                udpIp = jsonUdpIpIt->get<std::string>();
                Utils::IP::NormalizeIp(udpIp);
                uint16_t udpPort{0u};
                auto jsonUdpPortIt = data.find("otherUdpPort");
                if (
                        jsonUdpPortIt == data.end() ||
                        !Utils::Json::IsPositiveInteger(*jsonUdpPortIt)
                        ) {
                    MS_THROW_TYPE_ERROR("missing otherUdpPort ");
                }
                udpPort = jsonUdpPortIt->get<uint16_t>();
        ...
        switch (request->methodId) {
            case Channel::Request::MethodId::TRANSPORT_CONNECT: {
                ...
                if (this->dtlsTransport->SetRemoteFingerprint(dtlsRemoteFingerprint)) {
                   ...
                   MS_THROW_TYPE_ERROR("remoteFingerprint not pass , not run dtlsTransport");
                }
            ...
            case Channel::Request::MethodId::TRANSPORT_RESTART_ICE: {
                ...
                MS_DEBUG_DEV(
                        "WebRtcTransport ICE usernameFragment and password changed [id:%s]", this->transportId_.c_str());
    ...
    void WebRtcTransport::getConnectResult(json& data){
        switch (this->dtlsRole) {
            case RTC::DtlsTransport::Role::CLIENT:
    ...
    void WebRtcTransport::MayRunDtlsTransport() {
        ...
        if (this->dtlsTransport->GetLocalRole() == this->dtlsRole){
            MS_DEBUG_TAG(dtls,"Do nothing if we have the same local DTLS role as the DTLS transport");
            return;
        }
    ...
    inline void WebRtcTransport::OnIceServerSelectedTuple(
        ...
        Channel::Notifier::Emit(this->transportId_, "iceselectedtuplechange", data);
    }
    ...
    inline void WebRtcTransport::OnIceServerConnected(const RTC::IceServer * /*iceServer*/) {
        ...
        Channel::Notifier::Emit(this->transportId_, "icestatechange", data);
    ...
    inline void WebRtcTransport::OnDtlsTransportConnecting(const RTC::DtlsTransport * /*dtlsTransport*/) {
        ...
        Channel::Notifier::Emit(this->transportId_, "dtlsstatechange", data);
    }
